...
| `hackread` | **2026-08-06 以降 `403`。** サイト側が JS チャレンジを出しており HTTP クライアントでは原理的に通れない（2026-08-17 に停止）。⚠️ **`SOURCES` には残してある**＝蓄積済みの28件は画面に出続ける。判断・**試して駄目だったこと**・復旧の確認方法は [`docs/decisions.md`](docs/decisions.md) 項目22 |
...
- ⛔ **2026-08-17 に収集を停止した**（`disabled: true`）。**2026-08-06 以降 `403` が続き、
  ヘッダの調整では原理的に通れないことを実測で確認したため**（判断・**試して駄目だったこと**・
  復旧の確認方法は `docs/decisions.md` 項目22）。
  ⚠️ **自動では復活しない。** 復旧の確認は項目22 のコマンドを手で叩く。
- ⚠️ **ノイズの要否（項目11）は未決のまま。** 403 で運用が止まった期間は観測期間に数えられず、
  **判断材料が溜まっていない。** 復活させるときは改めて判断すること。
...
| `hackread` | 10件 | 5.6件/日 | **1.8日** | ⛔ **停止中**（403・項目22） |

- ⚠️ **BleepingComputer（2.4日）は、CI が2日止まると記事が消える。** フィードの件数が少なく
  **設定では増やせない**（`limit` を上げても相手が返さない）＝対策は cron 短縮しかない。
  **現状は cron 6時間のまま許容している。**
  （HackRead は 1.8日でさらに薄かったが、**403 で停止中**なので現在は該当しない。）
...
1. **集約（Node/tsx、ビルド前）**: GitHub Actions の cron（6時間ごと、`.github/workflows/update-and-deploy.yml`）が `scripts/aggregate.ts` を実行。7ソースを `FeedItem` に正規化 → 既存 feed.json とマージ → **未来日付のアイテムを除外**（フィードがイベント開催日等を公開日時に載せてくる対策。`docs/decisions.md` 項目25）→ id で重複排除 → publishedAt 降順ソート → **ソース別 `retentionMax` でトリム**（後述の全期間アーカイブ）→ **トリム後の最終アイテムに OGP サムネ補完 → 機械翻訳で日本語補完（いずれも後述）** → feed.json を上書き。
...
| 11 | **「無認証で使ってよい」と公式が言っているかの確認**（API を使うときだけ） | ⚠️ **動くかどうかとは別の問題。「動く＝許されている」ではない。** 公式ドキュメントで**無認証利用の可否とレート制限**を確認し、**根拠を `docs/decisions.md` に残す**。<br>・白の例: Qiita は「認証なしは IP ごとに 60回/時」と**明記**（項目19）<br>・見送った例: **技術的には完全に取得できたのに、公式が正規利用にエンタープライズ契約を求めていたため不採用**（項目22）<br>⚠️ **確認できないものは採用しない。** 資料が見つからない＝「禁止されていない」ではなく「**判断材料が無い**」。<br>⚠️ **公開 RSS / Atom はこの確認の対象外**（そもそも機械に読ませるためのもの）。対象は「API」と名の付くもの |
...
| 7 | **`.env.example` のコメント** | ⚠️ **読み書きできない。** そのソース／機能のトークンの説明が実態と合っているかを**依頼者に確認してもらう**（勝手に書き換えようとしない。「触ってはいけないもの」参照） |
...
**この節はユーザー設定（`~/.claude/CLAUDE.md`）へ移動した。** アプリの仕様ではなく
「依頼者と Claude Code の付き合い方」であり、**公開リポジトリに置く種類の情報ではない**ため
（かつここに置くと todaysec でしか効かず、他プロジェクトで元の状態に戻ってしまうため）。
**このプロジェクト固有の扱いを足したいときだけ、ここに書いて上書きする。**
