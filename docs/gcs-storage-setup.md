...
> ## ⚠️ 現在この構成は使用していません
>
> `feed.json` は **git 保管モード**（`src/data/feed.json` を CI がコミット）で運用しています。
> リポジトリ変数 `GCS_BUCKET` を設定していないため、ワークフローの GCS 関連ステップはすべて
> スキップされます。**この手順を実施しない限り、現在の動作は変わりません。**
>
> 将来 GCS へ移行する可能性があるため、手順は削除せず残しています。ただし以下に注意してください。
>
> - **本文中の GCP プロジェクト名・バケット名・サービスアカウント・WIF の設定は、
>   フォーク元（`satory074/todayai`）のものです。** そのまま実行しても権限がないため動きません。
>   流用する場合は自分の GCP プロジェクトで作り直し、リポジトリ名も `tomoraku5/todaysec` に
>   読み替えてください。
> - GCS の利用には**料金が発生する可能性があります**。現在の運用方針（費用 0 円）とは
>   前提が異なります。
