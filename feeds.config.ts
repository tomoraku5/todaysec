...
export interface FeedsConfig {
   ...
   *
   * ⚠️ **現在は無効（`disabled: true`）。2026-08-06 以降 `403` で取得できない。**
   * サイト側（Cloudflare）が **JavaScript の実行を求めるチャレンジ**を出しており、
   * HTTP クライアントでは原理的に通れない。**フィードだけでなくトップページも 403。**
   *
   * ⚠️ **回避策を思いついたら、まず `docs/decisions.md` 項目22 を読むこと。**
   * UA の変更・詐称、標準ヘッダの付与、別経路の利用は**すべて実測で否定済み**で、
   * 何を試して何が駄目だったかが記録してある。**同じ調査を繰り返さないため。**
   *
   ...
   * **ノイズを承知の上で採用**しており、除外の仕組みは意図的に作っていない（詳細は CLAUDE.md）。
   * ⚠️ **403 で運用が止まったため、除外の要否を判断する材料は溜まっていない。**
   *
   * ⚠️ **フィードが 10 件しか返さなかった**。実測 5.6件/日 なので**約1.8日分**しか遡れず、
   * **全ソースで最も余裕が薄かった**（BleepingComputer 15件＝約2.4日分がその次）。
   * ソース別の余裕は CLAUDE.md の「フィードから何日分遡れるか」の表が正。
   *
   * サムネはフィードに無く `enrichArticles` の og:image 補完に依存していたが、
   * **記事ページも 403 なので、復活させてもサムネは付かない**（画像なしの行になる）。
   */
  hackread: {
    rssUrls: string[];
    /** 1回に取り込む最大件数（**1 URL あたり**の取得窓。蓄積は retentionMax まで） */
...
export const feedsConfig: FeedsConfig = {
  ...
  hackread: {
    ...
    limit: 20,
    // 実測 5.6件/日なので 1000 件 ≒ 6ヶ月分。既定のままで足りる。
    retentionMax: 1000,
    // ⚠️ 403（サイト側の JS チャレンジ）で取得できないため停止中。
    //    ここを false に戻すだけで収集は再開する（src/lib/feed.ts の SOURCES には
    //    残してあるので、蓄積済みの記事は停止中も画面に出続ける）。
    //    判断と、試して駄目だった手段の一覧は docs/decisions.md 項目22。
    disabled: true,
  },
  cloudnative: {
